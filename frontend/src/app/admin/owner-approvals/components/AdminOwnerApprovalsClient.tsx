...
interface AdminOwnerApprovalsClientProps {
  initialApplications: OwnerDetail[]
  initialTotalPages: number
}

export default function AdminOwnerApprovalsClient({
  initialApplications,
  initialTotalPages,
}: AdminOwnerApprovalsClientProps) {
  ...
  const [applications, setApplications] = useState<OwnerDetail[]>(initialApplications);
  ...
  // Skip the first fetch for PENDING status since we already have the initial data
  useEffect(() => {
    if (sessionStatus === "authenticated" && session?.user?.roleName === "Admin") {
      if (activeTab !== 'PENDING' || page !== 0 || applications !== initialApplications) {
