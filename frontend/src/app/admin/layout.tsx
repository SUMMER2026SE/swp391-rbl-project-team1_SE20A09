...
export default function AdminLayout({
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/login");
    } else if (status === "authenticated" && session?.user?.roleName !== "Admin") {
      router.push("/");
    }
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 px-4 md:px-8 flex items-center justify-between shrink-0 sticky top-0 z-40">
          <div className="flex items-center gap-4">
            <button className="md:hidden text-slate-500 p-2 -ml-2 rounded-lg hover:bg-slate-100" onClick={() => setIsSidebarOpen(true)}>
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            <div className="flex items-center gap-3">
              {session?.user ? (
                ...
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=admin`} alt="Admin" className="h-9 w-9 rounded-full bg-emerald-100 border border-slate-200" />
              )}
              <div className="hidden md:block">
                <p className="text-sm font-semibold text-slate-900 leading-none">{session?.user ? `${session.user.lastName} ${session.user.firstName}` : "Admin SportHub"}</p>
