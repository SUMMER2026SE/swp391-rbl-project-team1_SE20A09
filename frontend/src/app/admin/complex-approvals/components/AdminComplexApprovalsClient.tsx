...
interface AdminComplexApprovalsClientProps {
  initialComplexes: ComplexResponse[]
  initialAmenities: Amenity[]
}

export default function AdminComplexApprovalsClient({
  initialComplexes,
  initialAmenities,
}: AdminComplexApprovalsClientProps) {
  ...
  // Refetch when activeTab changes, but skip the first fetch if it is the initial PENDING status
  useEffect(() => {
    if (activeTab !== 'PENDING' || complexes !== initialComplexes) {
