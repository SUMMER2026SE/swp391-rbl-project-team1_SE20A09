...
import { fetchAdminWalletBalance, fetchAdminWalletTransactions, WalletTransaction } from "@/lib/wallet-api";
  ...
  const loadWalletData = useCallback(async () => {
    ...
    try {
      const [balRes, txRes] = await Promise.all([
        ...
        fetchAdminWalletTransactions(page, size)
      ]);
      ...
      console.error(err);
      toast.error("Không thể tải thông tin ví hệ thống.");
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  const getTransactionBadge = (type: string) => {
    switch (type) {
      case "SERVICE_FEE_CREDIT":
        return <Badge className="bg-emerald-50 text-emerald-700 hover:bg-emerald-50 border-emerald-200">Thu phí dịch vụ</Badge>;
      case "REFUND_FEE_DEBIT":
        return <Badge className="bg-rose-50 text-rose-700 hover:bg-rose-50 border-rose-200">Hoàn phí dịch vụ</Badge>;
      default:
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2 bg-gradient-to-br from-indigo-600 to-purple-700 text-white shadow-xl border-none relative overflow-hidden group">
          ...
          <CardContent className="p-6 md:p-8 flex flex-col justify-between h-full min-h-[160px]">
            <div>
              <div className="flex items-center gap-2 text-indigo-100/90 text-sm font-semibold uppercase tracking-wider mb-2">
                <Landmark className="h-5 w-5" />
                Ví doanh thu hệ thống (Platform Wallet)
              </div>
              <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mt-2">
                {balance !== null ? `${balance.toLocaleString("vi-VN")} đ` : "---"}
              </h1>
            </div>
            <p className="text-xs text-indigo-100/70 mt-4">
              Đây là tổng số dư phí dịch vụ thu được từ toàn bộ các giao dịch đặt sân trên hệ thống.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 shadow-sm bg-white dark:bg-slate-900">
          <CardHeader className="pb-2">
            <CardTitle className="text-slate-600 text-xs font-semibold uppercase tracking-wider">Thông tin hệ thống</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center py-1.5 border-b border-slate-100">
              <span className="text-slate-500 text-sm">Chủ tài khoản</span>
              <span className="font-semibold text-sm text-slate-800">Admin Platform</span>
            </div>
            <div className="flex justify-between items-center py-1.5 border-b border-slate-100">
      ...
      <Card className="border border-slate-200 shadow-sm bg-white">
        <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
          <CardTitle className="text-lg font-bold text-slate-800 flex items-center gap-2">
            Lịch sử giao dịch phí dịch vụ (Ledger)
          </CardTitle>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-2">
              ...
              <span className="text-sm font-medium">Chưa có phí dịch vụ nào được thu.</span>
            </div>
          ) : (
