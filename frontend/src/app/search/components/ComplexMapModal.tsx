...
interface ComplexMapModalProps {
  isOpen: boolean
  onClose: () => void
...
export default function ComplexMapModal({ isOpen, onClose, complexes }: ComplexMapModalProps) {
  ...
  // Filter complexes with coordinates
      ...
      <div className="relative w-full max-w-4xl bg-white dark:bg-card rounded-3xl overflow-hidden shadow-2xl border border-gray-100 dark:border-border flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-gray-100 dark:border-border flex flex-col pr-16 bg-white dark:bg-card">
          <h3 className="text-xl font-extrabold text-gray-900 dark:text-white flex items-center gap-2">
            📍 Bản đồ Tổ Hợp Thể Thao
          </h3>
          <p className="text-xs text-muted-foreground mt-1">
            Hiển thị vị trí của {validComplexes.length} tổ hợp có định vị GPS
          </p>
        </div>
                    ...
                    <div className="w-60 overflow-hidden font-sans p-1">
                      ...
                      <div className="flex items-center gap-1 mb-1.5">
                        ...
                        <span className="text-xs font-bold text-gray-800 dark:text-gray-200">
                          {complex.averageRating ? complex.averageRating.toFixed(1) : '5.0'}
                        </span>
                      </div>
