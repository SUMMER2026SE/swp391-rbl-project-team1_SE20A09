...
public interface RefundExceptionRepository extends JpaRepository<RefundExceptionRequest, Integer> {
    ...
    Page<RefundExceptionRequest> findAllByStatusIn(List<RefundExceptionStatus> statuses, Pageable pageable);

    /** Danh sách request của 1 khách hàng — phân trang. */
    Page<RefundExceptionRequest> findByCustomerUserIdOrderByCreatedAtDesc(Integer userId, Pageable pageable);
     ...
     * Join qua booking → stadium → owner để lọc đúng sân của Owner đang đăng nhập.
     */
    @Query("""
